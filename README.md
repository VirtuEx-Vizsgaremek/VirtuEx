...
<img width="100%" alt="ER_diagram" src="https://github.com/user-attachments/assets/eee67522-a3a5-43ea-9c25-73bb633ffe95" />

## `currency`

**Description:** Stores all available currencies in the system, including both fiat currencies (USD, EUR) and cryptocurrencies (BTC, ETH). Each currency entry contains metadata such as symbol, name, decimal precision, and currency type classification.

| field      | type          | explanation                                                                                     |
| ---------- | ------------- | ----------------------------------------------------------------------------------------------- |
| id         | bigint        | **(PK)** Snowflake ID identifying the currency. Generated by the application, not the database. |
| created_at | timestamptz   | When the currency entry was created.                                                            |
| updated_at | timestamptz   | When the currency entry was last updated.                                                       |
| symbol     | varchar(255)  | Short symbol of the currency (e.g. BTC, USD).                                                   |
| name       | varchar(255)  | Human-readable name of the currency.                                                            |
| precision  | int           | Number of decimal places the currency supports.                                                 |
| type       | currency_type | Whether the currency is fiat or crypto.                                                         |

## `currency_history`

**Description:** Records historical price data for all currencies at specific timestamps. This table maintains a time-series record of price movements, enabling chart visualization and historical analysis. The combination of currency_id and timestamp forms a composite unique key.

| field       | type        | explanation                                        |
| ----------- | ----------- | -------------------------------------------------- |
| currency_id | bigint      | **(PK, FK)** Snowflake ID of the related currency. |
| timestamp   | timestamptz | **(PK)** Point in time for this price entry.       |
| price       | bigint      | Price at that timestamp, stored in smallest unit.  |

## `wallet`

**Description:** Represents a financial wallet associated with a user. Each user has one wallet that acts as a container for all their asset holdings across different currencies. Wallets track creation and modification timestamps for audit purposes.

| field      | type        | explanation                                   |
| ---------- | ----------- | --------------------------------------------- |
| id         | bigint      | **(PK)** Snowflake ID identifying the wallet. |
| created_at | timestamptz | When the wallet was created.                  |
| updated_at | timestamptz | When the wallet was last updated.             |

## `user`

| field        | type          | explanation                             |
| ------------ | ------------- | --------------------------------------- |
| id           | bigint        | Snowflake ID identifying the user.      |
| created_at   | timestamptz   | When the user account was created.      |
| updated_at   | timestamptz   | When the user account was last updated. |
| full_name    | varchar(1024) | User’s full display name.               |
| username     | varchar(32)   | Unique username.                        |
| email        | varchar(320)  | Unique email address.                   |
| password     | varchar(72)   | Hashed password.                        |
| bio          | varchar(256)  | Optional user biography.                |
| avatar       | varchar(40)   | Avatar identifier or hash.              |
| wallet_id    | bigint        | Snowflake ID of the user’s wallet.      |
| permissions  | bigint        | Bitmask representing user permissions.  |
| activated    | boolean       | Whether the account is activated.       |
| subscription | subscription  | Subscription tier of the user.          |

## `code`

**Description:** Temporary verification and authentication codes used for account activation, password resets, and email verification. Each code is time-limited with an expiration timestamp and is associated with a specific user. Code types differentiate between various purposes (activation, password reset, email verification, etc.).

| field      | type         | explanation                                       |
| ---------- | ------------ | ------------------------------------------------- |
| id         | bigint       | **(PK)** Snowflake ID identifying the code entry. |
| code       | varchar(255) | Verification or reset code sent to the user.      |
| type       | code_type    | Purpose of the code.                              |
| user_id    | bigint       | **(FK)** Snowflake ID of the related user.        |
| expires_at | timestamptz  | When the code expires.                            |

## `asset`

**Description:** Represents a holding of a specific currency within a user's wallet. Each asset record tracks the amount of a particular currency (BTC, USD, etc.) owned by a wallet. Assets are the core representation of user holdings and are updated during transactions, trades, and swaps.

| field       | type        | explanation                                  |
| ----------- | ----------- | -------------------------------------------- |
| id          | bigint      | **(PK)** Snowflake ID identifying the asset. |
| created_at  | timestamptz | When the asset entry was created.            |
| updated_at  | timestamptz | When the asset entry was last updated.       |
| wallet_id   | bigint      | **(FK)** Snowflake ID of the owning wallet.  |
| currency_id | bigint      | **(FK)** Snowflake ID of the currency.       |
| amount      | bigint      | Amount held, in smallest currency unit.      |

## `transaction`

**Description:** Immutable record of all asset movements within the system, including trades, deposits, withdrawals, swaps, and transfers. Each transaction is tracked with creation and update timestamps, linked to an asset, and maintains a status indicating its current state (pending, completed, failed, etc.). The direction field indicates whether the transaction represents incoming or outgoing movement of funds.

| field      | type                  | explanation                                        |
| ---------- | --------------------- | -------------------------------------------------- |
| id         | bigint                | **(PK)** Snowflake ID identifying the transaction. |
| created_at | timestamptz           | When the transaction was created.                  |
| updated_at | timestamptz           | When the transaction was last updated.             |
| asset_id   | bigint                | **(FK)** Snowflake ID of the related asset.        |
...
These endpoints are responsible for registration, login and 2fa.
...
These endpoints are responsible for profile management (F-03, F-13, F-16).
